...
// EncodeMultiplicationResponse encodes responses from the "calc" service
// "multiplication" endpoint.
func EncodeMultiplicationResponse(ctx context.Context, v interface{}, hdr, trlr *metadata.MD) (interface{}, error) {
	result, ok := v.(int)
	if !ok {
		return nil, goagrpc.ErrInvalidType("calc", "multiplication", "int", v)
	}
	resp := NewMultiplicationResponse(result)
	return resp, nil
}

// DecodeMultiplicationRequest decodes requests sent to "calc" service
// "multiplication" endpoint.
func DecodeMultiplicationRequest(ctx context.Context, v interface{}, md metadata.MD) (interface{}, error) {
	var (
		message *calcpb.MultiplicationRequest
		ok      bool
	)
	{
		if message, ok = v.(*calcpb.MultiplicationRequest); !ok {
			return nil, goagrpc.ErrInvalidType("calc", "multiplication", "*calcpb.MultiplicationRequest", v)
		}
	}
	var payload *calc.MultiplicationPayload
	{
		payload = NewMultiplicationPayload(message)
	}
	return payload, nil
}
