...
func UsageCommands() string {
	return `calc (add|minus|multiplication)
`
}

// UsageExamples produces an example of a valid invocation of the CLI tool.
func UsageExamples() string {
	return os.Args[0] + ` calc add --a 8133055152903002499 --b 3219793201326175278` + "\n" +
		""
}
...
func ParseEndpoint(
	...
	var (
		...
		calcMultiplicationFlags = flag.NewFlagSet("multiplication", flag.ExitOnError)
		calcMultiplicationAFlag = calcMultiplicationFlags.String("a", "REQUIRED", "Left operand")
		calcMultiplicationBFlag = calcMultiplicationFlags.String("b", "REQUIRED", "Right operand")
	)
	calcFlags.Usage = calcUsage
	calcAddFlags.Usage = calcAddUsage
	calcMinusFlags.Usage = calcMinusUsage
	calcMultiplicationFlags.Usage = calcMultiplicationUsage
	...
	)
	{
		...
		case "calc":
			...
			case "multiplication":
				endpoint = c.Multiplication()
				data, err = calcc.BuildMultiplicationPayload(*calcMultiplicationAFlag, *calcMultiplicationBFlag)
			}
		}
	}
	if err != nil {
...
COMMAND:
    ...
    multiplication: Multiplication implements multiplication.
...
Example:
    `+os.Args[0]+` calc add --a 8133055152903002499 --b 3219793201326175278
`, os.Args[0])
}
...
Example:
    `+os.Args[0]+` calc minus --a 4212629202012168060 --b 1698882017578366363
`, os.Args[0])
}

func calcMultiplicationUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] calc multiplication -a INT -b INT
...
Example:
    `+os.Args[0]+` calc multiplication --a 8228676432890045784 --b 5308928064408640979
`, os.Args[0])
}
