...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			{"Add", "GET", "/add/{a}/{b}"},
			{"Minus", "GET", "/minus/{a}/{b}"},
			{"Multiplication", "GET", "/multiplication/{a}/{b}"},
			{"./gen/http/openapi.json", "GET", "/openapi.json"},
		},
		Add:            NewAddHandler(e.Add, mux, decoder, encoder, errhandler, formatter),
		Minus:          NewMinusHandler(e.Minus, mux, decoder, encoder, errhandler, formatter),
		Multiplication: NewMultiplicationHandler(e.Multiplication, mux, decoder, encoder, errhandler, formatter),
	}
}
...
func (s *Server) Use(m func(http.Handler) http.Handler) {
	s.Add = m(s.Add)
	s.Minus = m(s.Minus)
	s.Multiplication = m(s.Multiplication)
}
...
func Mount(mux goahttp.Muxer, h *Server) {
	...
	MountMultiplicationHandler(mux, h.Multiplication)
...
// MountMultiplicationHandler configures the mux to serve the "calc" service
// "multiplication" endpoint.
func MountMultiplicationHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := h.(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("GET", "/multiplication/{a}/{b}", f)
}

// NewMultiplicationHandler creates a HTTP handler which loads the HTTP request
// and calls the "calc" service "multiplication" endpoint.
func NewMultiplicationHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
	encoder func(context.Context, http.ResponseWriter) goahttp.Encoder,
	...
	var (
		decodeRequest  = DecodeMultiplicationRequest(mux, decoder)
		encodeResponse = EncodeMultiplicationResponse(encoder)
		encodeError    = goahttp.ErrorEncoder(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "multiplication")
		ctx = context.WithValue(ctx, goa.ServiceKey, "calc")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil {
