...
// BuildMultiplicationRequest instantiates a HTTP request object with method
// and path set to call the "calc" service "multiplication" endpoint
func (c *Client) BuildMultiplicationRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	var (
		a int
		b int
	)
	{
		...
		if !ok {
			return nil, goahttp.ErrInvalidType("calc", "multiplication", "*calc.MultiplicationPayload", v)
		}
		a = p.A
		b = p.B
	}
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: MultiplicationCalcPath(a, b)}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("calc", "multiplication", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// DecodeMultiplicationResponse returns a decoder for responses returned by the
// calc multiplication endpoint. restoreBody controls whether the response body
// should be restored after having been read.
func DecodeMultiplicationResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		if restoreBody {
			b, err := ioutil.ReadAll(resp.Body)
			if err != nil {
		...
		case http.StatusOK:
			...
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("calc", "multiplication", err)
			}
			return body, nil
		default:
			body, _ := ioutil.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("calc", "multiplication", resp.StatusCode, string(body))
		}
	}
}
