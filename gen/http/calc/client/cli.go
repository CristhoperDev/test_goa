...
func BuildMultiplicationPayload(calcMultiplicationA string, calcMultiplicationB string) (*calc.MultiplicationPayload, error) {
	var err error
	var a int
	{
		var v int64
		v, err = strconv.ParseInt(calcMultiplicationA, 10, 64)
		a = int(v)
		if err != nil {
			return nil, fmt.Errorf("invalid value for a, must be INT")
		}
	...
	{
		var v int64
		v, err = strconv.ParseInt(calcMultiplicationB, 10, 64)
		b = int(v)
		if err != nil {
			return nil, fmt.Errorf("invalid value for b, must be INT")
		}
	}
	payload := &calc.MultiplicationPayload{
		A: a,
		B: b,
	}
	return payload, nil
