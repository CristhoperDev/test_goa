...
type Server struct {
	...
	MultiplicationH goagrpc.UnaryHandler
}
...
func New(e *calc.Endpoints, uh goagrpc.UnaryHandler) *Server {
	return &Server{
		...
		MultiplicationH: NewMultiplicationHandler(e.Multiplication, uh),
	}
}
...
// NewMultiplicationHandler creates a gRPC handler which serves the "calc"
// service "multiplication" endpoint.
func NewMultiplicationHandler(endpoint goa.Endpoint, h goagrpc.UnaryHandler) goagrpc.UnaryHandler {
	if h == nil {
		h = goagrpc.NewUnaryHandler(endpoint, DecodeMultiplicationRequest, EncodeMultiplicationResponse)
	}
	return h
}

// Multiplication implements the "Multiplication" method in calcpb.CalcServer
// interface.
func (s *Server) Multiplication(ctx context.Context, message *calcpb.MultiplicationRequest) (*calcpb.MultiplicationResponse, error) {
	ctx = context.WithValue(ctx, goa.MethodKey, "multiplication")
	ctx = context.WithValue(ctx, goa.ServiceKey, "calc")
	resp, err := s.MultiplicationH.Handle(ctx, message)
	if err != nil {
		return nil, goagrpc.EncodeError(err)
	}
	return resp.(*calcpb.MultiplicationResponse), nil
}
